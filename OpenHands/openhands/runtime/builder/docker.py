...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def image_exists(self, image_name: str, pull_from_repo: bool = True) -> bool:
        ...
        try to pull the same image simultaneously.
        ...
        # Fast path: check if image exists locally (no lock needed)
        try:
        ...
        # Slow path: need to pull, use file lock to prevent race conditions
        from openhands.runtime.utils.image_lock import ImageLock

        lock = ImageLock(image_name)
        if not lock.acquire(timeout=300.0):
            logger.warning(
                f'Could not acquire lock for image {image_name}, proceeding anyway'
            )

        try:
            # Double-check: another process may have pulled the image while we waited
            try:
                self.docker_client.images.get(image_name)
